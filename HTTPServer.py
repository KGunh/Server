...
def a_star(start_node, end_node, allNodes, isCCTV):
    allLinks = SQLInstance().getLinks()
    weights = SQLInstance().getCCTVs() if isCCTV else SQLInstance().getCCTVs()
...
def a_route(from_node, to_node, allNodes, isCCTV, result_list):
    ...
    # 신호등 가중치 구현 후 True 부분 변경할 것
    result_path, node_set = a_star(Node(int(nodeId[0]), from_node), Node(int(to_nodeId[0]), to_node), allNodes, isCCTV)
    param_format = {
        'features' :[
            {
                'geometry' : {
